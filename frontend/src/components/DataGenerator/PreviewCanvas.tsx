...
import { loadImage, drawWithEdgeBlend, getBagBoundary, drawYOLOBoxes, cropImageToRect } from '@/utils/imageProcessing'
import { generatePlacement, randomChoice } from '@/utils/randomizer'
import type { CanvasObject, YOLOAnnotation, YOLOBoxRaw, CropRect } from '@/types'
...
export default function PreviewCanvas({ currentBgId, selectedObjectIds, mode }: PreviewCanvasProps) {
  ...
  const { params, currentDatasetId, addSamples, datasets } = useDatasetStore()
  ...
  const renderCanvas = useCallback(async () => {
    ...
    if (bgImgRef.current && fitRectRef.current) {
      ...
      const activeBoundary = userBoundary || bagBoundary
      if (activeBoundary) {
        if (userBoundary) {
          ctx.strokeStyle = 'rgba(34, 197, 94, 0.8)'
          ctx.lineWidth = 3
          ctx.setLineDash([])
          const bx = drawX + activeBoundary.x * scale
          const by = drawY + activeBoundary.y * scale
          const bw = activeBoundary.width * scale
          const bh = activeBoundary.height * scale
  ...
  }, [canvasObjects, bgImgRef, selectedObjId, params, objects, bagBoundary, userBoundary, rectDrawing, yoloBoxes, cropRect])
  ...
  }, [rectMode, previewPan, previewZoom])

  const handleCanvasMouseMove = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
    if (rectMode && rectDrawing) {
      const canvas = canvasRef.current
      if (!canvas) return
      const rect = canvas.getBoundingClientRect()
      const fx = (e.clientX - rect.left) / previewZoom
  ...
  }, [rectMode, rectDrawing, isPanning, panStart, previewZoom])
  ...
  const handleCanvasMouseUp = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    setIsPanning(false)
  }, [rectMode, rectDrawing, canvasToImageCoords, previewZoom])
  ...
  }, [rectMode, mode, canvasObjects, fitToImageCoords])
  ...
  const handleDrop = useCallback((e: React.DragEvent) => {
    ...
    img.onload = () => {
      ...
      const newObj: CanvasObject = {
        ...
        scaleY: 1,
        opacity: 1,
        imageData: foreignObj.cutoutImage || foreignObj.thumbnail,
      }
  ...
  const getActiveObjects = useCallback(() => {
    if (selectedObjectIds.length > 0) return objects.filter((o) => selectedObjectIds.includes(o.id))
    return objects
  ...
  const getBoundary = useCallback((): BagRect | undefined => {
    ...
    return undefined
  }, [bagBoundary, userBoundary])

  const generateAutoPreview = useCallback(() => {
    if (!bg || objects.length === 0) return
    ...
    const loadAndPlace = selectedObjects.map(async (foreignObj) => {
      ...
      try {
        ...
        const placement = generatePlacement(imgW, imgH, img.width, img.height, params, boundary)
        return {
    ...
    for (let i = 0; i < params.totalCount; i++) {
      ...
      for (const foreignObj of selectedObjects) {
        ...
        try {
          ...
          const catId = categories.indexOf(foreignObj.category)
          const classId = catId >= 0 ? catId : 0
  ...
  const handleManualGenerate = useCallback(async () => {
    ...
    for (const co of canvasObjects) {
      ...
      try {
        ...
        let bgMean: number | undefined
        try {
          const sx = Math.max(0, Math.floor(co.x))
          const sy = Math.max(0, Math.floor(co.y))
          const sw = Math.min(Math.ceil(co.width), imgW - sx)
          const sh = Math.min(Math.ceil(co.height), imgH - sy)
          if (sw > 1 && sh > 1) {
            const bgSample = genCtx.getImageData(sx, sy, sw, sh)
            let sum = 0, count = 0
            for (let pi = 0; pi < bgSample.data.length; pi += 4) {
  ...
  const canGenerate = !!bg && objects.length > 0
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      >
        ...
        >
          <canvas
            ...
            style={{ cursor: rectMode ? 'crosshair' : 'default' }}
          />
        </div>
      </div>
      ...
      <div className="p-3 border-t border-gray-200 dark:border-gray-800 flex items-center gap-2 flex-wrap">
        ...
        <button
          onClick={() => { setRectMode(!rectMode); setRectDrawing(null) }}
          className={`px-2.5 py-1 rounded text-xs transition-all ${rectMode ? 'bg-green-500 text-white' : 'btn-secondary'}`}
        >
          📐 {rectMode ? '绘制中...' : '画矩形限定区域'}
        </button>
        {userBoundary && (
          <button onClick={() => { setUserBoundary(null); renderCanvas() }} className="btn-secondary text-xs px-2 py-1">
            ✕ 清除自定义区域
          </button>
        )}
        {yoloBoxes.length > 0 && (
          <span className="text-xs text-red-400">📦 {yoloBoxes.length} 个YOLO标注</span>
        )}
        <div className="h-5 w-px bg-gray-200 dark:bg-gray-700" />
        {mode === 'auto' ? (
          <>
